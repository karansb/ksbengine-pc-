...
namespace ksbengine{namespace coreengine{
	objfileloader::objfileloader(char *filepath){
		...
		std::cout << texcoordsdata.size() << std::endl;
		//vertexindexfilled;
		...
		texcoord = new GLfloat[vertexdataindex.size()/3*2];
		...
		for (unsigned int i = 0; i < vertexdataindex.size(); i += 3){
			...
			if (texcoordsdata.size()!=0){
				texcoord[texcoordcount] = texcoordsdata[2 * (vertexdataindex[i + 1] - 1) + 0];
				texcoordcount++;
				texcoord[texcoordcount] = texcoordsdata[2 * (vertexdataindex[i + 1] - 1) + 1];
				texcoordcount++;
			}
	...
	void objfileloader::createbuffer(){
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*vertexdataindexsize/3*2, texcoord, GL_STATIC_DRAW);
	}
